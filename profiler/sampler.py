...
train_loader_gpu0 = torch.utils.data.DataLoader(
    train_idx_parallel[0], batch_size=1024, shuffle=True, drop_last=True
)
train_loader_gpu1 = torch.utils.data.DataLoader(
    train_idx_parallel[1], batch_size=1024, shuffle=True, drop_last=True
)
...
def reindex_by_degree(
    adj_csr: quiver.CSRTopo, gpu_portion: float
...
def hit_ratio_analysis_on_clique(gpu_portion: float):
    ...
    for i in range(2):
        batch_nids_gpui_path = get_profiler_data_save_path("nids_gpu" + str(i) + ".csv")
    ...
    res_df.to_csv(
        get_profiler_data_save_path("hit_analysis_" + str(gpu_portion * 100) + "%.csv"),
        index=False,
    )
